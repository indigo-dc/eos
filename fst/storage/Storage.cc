...
Storage::Boot(FileSystem* fs)
{
  ...
  if (resyncdisk && (fs->GetPath()[0] == '/')) {
    ...
    eos_info("msg=\"skipped disk synchronisation\" fsid=%lu",
             (unsigned long) fsid);
  }
...
Storage::StartBoot(void* pp)
{
  if (pp) {
    ...
    info->storage->mBootingSet.erase(info->filesystem->GetId());
